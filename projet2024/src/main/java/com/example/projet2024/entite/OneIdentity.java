...
@Setter
@Entity
@Table(name = "OneIdentity")
public class OneIdentity {
    @Id
    @Column(name = "OneIdentityId")
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long oneIdentityId;

    @Basic
    @Column(name = "Client")
    private String client;

    @OneToMany(mappedBy = "oneIdentity", cascade = CascadeType.ALL, orphanRemoval = true)
    @JsonManagedReference("oneidentity-licence")
