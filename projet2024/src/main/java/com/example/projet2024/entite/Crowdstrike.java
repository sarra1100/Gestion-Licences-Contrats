...
@Setter
@Entity
@Table(name = "Crowdstrike")
public class Crowdstrike {
    @Id
    @Column(name = "CrowdstrikeId")
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long crowdstrikeid;

    @Basic
    @Column(name = "Client")
    private String client;

    @OneToMany(mappedBy = "crowdstrike", cascade = CascadeType.ALL, orphanRemoval = true)
    @JsonManagedReference("crowdstrike-licence")
