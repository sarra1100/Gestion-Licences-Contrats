...
@Setter
@Entity
@Table(name = "Bitdefender")
public class Bitdefender {
    @Id
    @Column(name = "BitdefenderId")
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long bitdefenderId;

    @Basic
    @Column(name = "Client")
    private String client;

    @OneToMany(mappedBy = "bitdefender", cascade = CascadeType.ALL, orphanRemoval = true)
    @JsonManagedReference("bitdefender-licence")
