...
@Setter
@Entity
@Table(name = "Alwarebytes")
public class Alwarebytes {
    @Id
    @Column(name = "AlwarebytesId")
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long alwarebytesId;

    @Basic
    @Column(name = "Client")
    private String client;

    @OneToMany(mappedBy = "alwarebytes", cascade = CascadeType.ALL, orphanRemoval = true)
    @JsonManagedReference("alwarebytes-licence")
