...
@Setter
@Entity
@Table(name = "SentineIOne")
public class SentineIOne {
    @Id
    @Column(name = "SentineIOneId")
    @GeneratedValue(strategy = GenerationType.IDENTITY)
    private Long sentineIOneId;

    @Basic
    @Column(name = "Client")
    private String client;

    @OneToMany(mappedBy = "sentineIOne", cascade = CascadeType.ALL, orphanRemoval = true)
    @JsonManagedReference("sentineIOne-licence")
