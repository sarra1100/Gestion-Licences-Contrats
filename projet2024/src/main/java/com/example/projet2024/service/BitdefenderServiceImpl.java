...
import com.example.projet2024.repository.BitdefenderRepo;
...
@Service
public class BitdefenderServiceImpl implements IBitdefenderService{
    @Autowired
    private BitdefenderRepo bitdefenderRepo;
    @Autowired
    ...
    @Override
    public Bitdefender addBitdefender(Bitdefender bitdefender) {
        if (bitdefender.getLicences() != null) {
            for (LicenceFortinet licence : bitdefender.getLicences()) {
                licence.setBitdefender(bitdefender);
            }
        }
        return bitdefenderRepo.save(bitdefender);
    }

    @Override
    public Bitdefender updateBitdefender(Bitdefender bitdefender) {
        return bitdefenderRepo.save(bitdefender);
    }

    @Override
    public Bitdefender retrieveBitdefender(Long bitdefenderId) {
        return bitdefenderRepo.findById(bitdefenderId).orElse(null);
    }

    @Override
    public List<Bitdefender> retrieveAllBitdefenders() {
        return bitdefenderRepo.findAll();
    }

    @Override
    public void deleteById(Long bitdefenderId) {
        bitdefenderRepo.deleteById(bitdefenderId);
    }

    @Override
    public void activate(Long id) {
        Bitdefender bitdefender = bitdefenderRepo.findById(id).orElse(null);
        if (bitdefender != null) {
            bitdefender.setApprouve(!bitdefender.isApprouve());
            bitdefenderRepo.save(bitdefender);
            System.out.println("Bitdefender ID: " + id + " est maintenant " + bitdefender.isApprouve());
        }
    }

    @Override
    public Bitdefender updateBitdefenderFile(Long bitdefenderId, String fichier, String fichierOriginalName) {
        Bitdefender bitdefender = bitdefenderRepo.findById(bitdefenderId).orElse(null);
        if (bitdefender != null) {
            bitdefender.setFichier(fichier);
            bitdefender.setFichierOriginalName(fichierOriginalName);
            return bitdefenderRepo.save(bitdefender);
        }
        return null;
    }
    ...
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringBitdefenders() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findBitdefenderLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Bitdefender bitdefender = licence.getBitdefender();
                if (bitdefender != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = bitdefender.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getBitdefender(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Bitdefender bitdefender, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = bitdefender.getMailAdmin();
        List<String> ccMails = bitdefender.getCcMail();

        String sujet = " Licences Bitdefender expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(bitdefender.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences Bitdefender expirant " + delai);
        } catch (Exception e) {
            System.err.println("Erreur envoi email Bitdefender: " + e.getMessage());
        }
    }
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Bitdefender bitdefender = licence.getBitdefender();
        if (bitdefender == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Bitdefender associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence Bitdefender expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(bitdefender.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(bitdefender.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Bitdefender expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    bitdefender.getMailAdmin(),
                    bitdefender.getCcMail() != null ? bitdefender.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence Bitdefender expirant " + timeframe);
        } catch (Exception e) {
