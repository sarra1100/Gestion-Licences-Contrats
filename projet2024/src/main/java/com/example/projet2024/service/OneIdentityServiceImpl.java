...
import com.example.projet2024.repository.OneIdentityRepo;
...
@Service
public class OneIdentityServiceImpl implements IOneIdentityService{
    @Autowired
    private OneIdentityRepo oneIdentityRepo;
    @Autowired
    ...
    @Override
    public OneIdentity addOneIdentity(OneIdentity oneIdentity) {
        if (oneIdentity.getLicences() != null) {
            for (LicenceFortinet licence : oneIdentity.getLicences()) {
                licence.setOneIdentity(oneIdentity);
            }
        }
        return oneIdentityRepo.save(oneIdentity);
    }

    @Override
    public OneIdentity updateOneIdentity(OneIdentity oneIdentity) {
        return oneIdentityRepo.save(oneIdentity);
    }

    @Override
    public OneIdentity updateOneIdentityFile(OneIdentity oneIdentity) {
        OneIdentity existing = oneIdentityRepo.findById(oneIdentity.getOneIdentityId()).orElse(null);
        if (existing != null) {
            existing.setFichier(oneIdentity.getFichier());
            existing.setFichierOriginalName(oneIdentity.getFichierOriginalName());
            return oneIdentityRepo.save(existing);
        }
        return null;
    }

    @Override
    public OneIdentity retrieveOneIdentity(Long oneIdentityId) {
        return oneIdentityRepo.findById(oneIdentityId).orElse(null);
    }

    @Override
    public List<OneIdentity> retrieveAllOneIdentitys() {
        return oneIdentityRepo.findAll();
    }

    @Override
    public void deleteById(Long oneIdentityId) {
        oneIdentityRepo.deleteById(oneIdentityId);
    }

    @Override
    public void activate(Long id) {
        OneIdentity oneIdentity = oneIdentityRepo.findById(id).orElse(null);
        if (oneIdentity != null) {
            oneIdentity.setApprouve(!oneIdentity.isApprouve());
            oneIdentityRepo.save(oneIdentity);
            System.out.println("OneIdentity ID: " + id + " est maintenant " + oneIdentity.isApprouve());
        }
    }

    // Méthode pour vérifier la date d'expiration - MÊME LOGIQUE QUE ALWAREBYTES
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringOneIdentitys() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findOneIdentityLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                OneIdentity oneIdentity = licence.getOneIdentity();
                if (oneIdentity != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = oneIdentity.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getOneIdentity(), licences, timeframeDisplay);
            }
        }
    }
    ...
    // MODIFIÉ POUR PRENDRE LE DELAI EN PARAMÈTRE (comme Alwarebytes)
    private void sendGroupedExpirationEmails(OneIdentity oneIdentity, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = oneIdentity.getMailAdmin();
        List<String> ccMails = oneIdentity.getCcMail();

        String sujet = " Licences OneIdentity expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(oneIdentity.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences OneIdentity expirant " + delai);
        } catch (Exception e) {
            System.err.println("Erreur envoi email OneIdentity: " + e.getMessage());
        }
    }
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        OneIdentity oneIdentity = licence.getOneIdentity();
        if (oneIdentity == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet OneIdentity associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence OneIdentity expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(oneIdentity.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(oneIdentity.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence OneIdentity expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    oneIdentity.getMailAdmin(),
                    oneIdentity.getCcMail() != null ? oneIdentity.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence OneIdentity expirant " + timeframe);
        } catch (Exception e) {
