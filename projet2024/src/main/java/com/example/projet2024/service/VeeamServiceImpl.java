...
import com.example.projet2024.repository.VeeamRepo;
...
@Service
public class VeeamServiceImpl implements IVeeamService {
    ...
    @Override
    public Veeam addVeeam(Veeam veeam) {
        if (veeam.getLicences() != null) {
            for (LicenceFortinet licence : veeam.getLicences()) {
                licence.setVeeam(veeam);
            }
        }
        return veeamRepo.save(veeam);
    }

    @Override
    public Veeam updateVeeam(Veeam veeam) {
        return veeamRepo.save(veeam);
    }

    @Override
    public Veeam retrieveVeeam(Long veeamId) {
        return veeamRepo.findById(veeamId).orElse(null);
    }

    @Override
    public List<Veeam> retrieveAllVeeams() {
        return veeamRepo.findAll();
    }

    @Override
    public void deleteById(Long veeamId) {
        veeamRepo.deleteById(veeamId);
    }

    @Override
    public void activate(Long id) {
        Veeam veeam = veeamRepo.findById(id).orElse(null);
        if (veeam != null) {
            veeam.setApprouve(!veeam.isApprouve());
            veeamRepo.save(veeam);
            System.out.println("Veeam ID: " + id + " est maintenant " + veeam.isApprouve());
        }
    }

    @Override
    public Veeam updateVeeamFile(Long veeamId, String fichier, String fichierOriginalName) {
        Veeam veeam = veeamRepo.findById(veeamId).orElse(null);
        if (veeam != null) {
            veeam.setFichier(fichier);
            veeam.setFichierOriginalName(fichierOriginalName);
            return veeamRepo.save(veeam);
        }
        return null;
    }
    ...
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringVeeams() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findVeeamLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Veeam veeam = licence.getVeeam();
                if (veeam != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = veeam.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getVeeam(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Veeam veeam, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = veeam.getMailAdmin();
        List<String> ccMails = veeam.getCcMail();

        String sujet = "Licences Veeam expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(veeam.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences Veeam expirant " + delai);
        } catch (Exception e) {
            System.err.println("Erreur envoi email Veeam: " + e.getMessage());
        }
    }
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Veeam veeam = licence.getVeeam();
        if (veeam == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Veeam associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence Veeam expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(veeam.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(veeam.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Veeam expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    veeam.getMailAdmin(),
                    veeam.getCcMail() != null ? veeam.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence Veeam expirant " + timeframe);
        } catch (Exception e) {
            throw new RuntimeException("Erreur lors de l'envoi du mail individuel Veeam", e);
        }
    }
}