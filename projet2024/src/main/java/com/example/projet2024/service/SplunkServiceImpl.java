...
import com.example.projet2024.repository.SplunkRepo;
...
public class SplunkServiceImpl implements ISplunkService {
    ...
    @Override
    public Splunk addSplunk(Splunk splunk) {
        if (splunk.getLicences() != null) {
            for (LicenceFortinet licence : splunk.getLicences()) {
                licence.setSplunk(splunk);
            }
        }
        return splunkRepo.save(splunk);
    }

    @Override
    public Splunk updateSplunk(Splunk splunk) {
        return splunkRepo.save(splunk);
    }

    @Override
    public Splunk updateSplunkFile(Splunk splunk) {
        Splunk existing = splunkRepo.findById(splunk.getSplunkid()).orElse(null);
        if (existing != null) {
            existing.setFichier(splunk.getFichier());
            existing.setFichierOriginalName(splunk.getFichierOriginalName());
            return splunkRepo.save(existing);
        }
        return null;
    }

    @Override
    public Splunk retrieveSplunk(Long splunkId) {
        return splunkRepo.findById(splunkId).orElse(null);
    }

    @Override
    public List<Splunk> retrieveAllSplunks() {
        return splunkRepo.findAll();
    }

    @Override
    public void deleteById(Long splunkId) {
        splunkRepo.deleteById(splunkId);
    }

    @Override
    public void activate(Long id) {
        Splunk splunk = splunkRepo.findById(id).orElse(null);
        if (splunk != null) {
            splunk.setApprouve(!splunk.isApprouve());
            splunkRepo.save(splunk);
            System.out.println("Splunk ID: " + id + " est maintenant " + splunk.isApprouve());
        }
    }

    // Méthode pour vérifier la date d'expiration
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringSplunks() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findSplunkLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Splunk splunk = licence.getSplunk();
                if (splunk != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = splunk.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getSplunk(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Splunk splunk, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = splunk.getMailAdmin();
        List<String> ccMails = splunk.getCcMail();

        String sujet = " Licences Splunk expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(splunk.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences Splunk expirant " + delai);
        } catch (Exception e) {
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Splunk splunk = licence.getSplunk();
        if (splunk == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Splunk associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence Splunk expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(splunk.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(splunk.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Splunk expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    splunk.getMailAdmin(),
                    splunk.getCcMail() != null ? splunk.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence Splunk expirant " + timeframe);
        } catch (Exception e) {
