...
public class VaronisServiceImpl implements IVaronisService {
    ...
    @Override
    public void updateVaronisFile(Long id, String fichier, String fichierOriginalName) {
        Varonis varonis = varonisRepo.findById(id).orElse(null);
        if (varonis != null) {
            varonis.setFichier(fichier);
            varonis.setFichierOriginalName(fichierOriginalName);
            varonisRepo.save(varonis);
        }
    }
