...
import com.example.projet2024.repository.FortraRepo;
...
@Service
public class FortraServiceImpl implements IFortraService {
    @Autowired
    private FortraRepo fortraRepo;
    @Autowired
    ...
    @Override
    public Fortra addFortra(Fortra fortra) {
        if (fortra.getLicences() != null) {
            for (LicenceFortinet licence : fortra.getLicences()) {
                licence.setFortra(fortra);
            }
        }
        return fortraRepo.save(fortra);
    }

    @Override
    public Fortra updateFortra(Fortra fortra) {
        return fortraRepo.save(fortra);
    }

    @Override
    public Fortra updateFortraFile(Fortra fortra) {
        return fortraRepo.save(fortra);
    }

    @Override
    public Fortra retrieveFortra(Long fortraId) {
        return fortraRepo.findById(fortraId).orElse(null);
    }

    @Override
    public List<Fortra> retrieveAllFortras() {
        return fortraRepo.findAll();
    }

    @Override
    public void deleteById(Long fortraId) {
        fortraRepo.deleteById(fortraId);
    }

    @Override
    public void activate(Long id) {
        Fortra fortra = fortraRepo.findById(id).orElse(null);
        if (fortra != null) {
            fortra.setApprouve(!fortra.isApprouve());
            fortraRepo.save(fortra);
            System.out.println("Fortra ID: " + id + " est maintenant " + fortra.isApprouve());
        }
    }

    // Méthode pour vérifier la date d'expiration
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringFortras() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findFortraLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Fortra fortra = licence.getFortra();
                if (fortra != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = fortra.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getFortra(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Fortra fortra, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = fortra.getMailAdmin();
        List<String> ccMails = fortra.getCcMail();

        String sujet = "Licences Fortra expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(fortra.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences Fortra expirant " + delai);
        } catch (Exception e) {
            System.err.println("Erreur envoi email Fortra: " + e.getMessage());
        }
    }
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Fortra fortra = licence.getFortra();
        if (fortra == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Fortra associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(fortra.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(fortra.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Fortra expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    fortra.getMailAdmin(),
                    fortra.getCcMail() != null ? fortra.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence Fortra expirant " + timeframe);
        } catch (Exception e) {
            throw new RuntimeException("Erreur lors de l'envoi du mail individuel Fortra", e);
        }
    }
}