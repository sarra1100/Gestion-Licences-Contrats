...
import com.example.projet2024.repository.Rapid7Repo;
...
@Service
public class Rapid7ServiceImpl implements IRapid7Service {

    @Autowired
    private Rapid7Repo rapid7Repo;
    @Autowired
    ...
    @Override
    public Rapid7 addRapid7(Rapid7 rapid7) {
        if (rapid7.getLicences() != null) {
            for (LicenceFortinet licence : rapid7.getLicences()) {
                licence.setRapid7(rapid7);
            }
        }
        return rapid7Repo.save(rapid7);
    }

    @Override
    public Rapid7 updateRapid7(Rapid7 rapid7) {
        return rapid7Repo.save(rapid7);
    }

    @Override
    public Rapid7 retrieveRapid7(Long rapid7Id) {
        return rapid7Repo.findById(rapid7Id).orElse(null);
    }

    @Override
    public List<Rapid7> retrieveAllRapid7s() {
        return rapid7Repo.findAll();
    }

    @Override
    public void deleteById(Long rapid7Id) {
        rapid7Repo.deleteById(rapid7Id);
    }

    @Override
    public void activate(Long id) {
        Rapid7 rapid7 = rapid7Repo.findById(id).orElse(null);
        if (rapid7 != null) {
            rapid7.setApprouve(!rapid7.isApprouve());
            rapid7Repo.save(rapid7);
            System.out.println("Rapid7 ID: " + id + " est maintenant " + rapid7.isApprouve());
        }
    }

    @Override
    public Rapid7 updateRapid7File(Long rapid7Id, String fichier, String fichierOriginalName) {
        Rapid7 rapid7 = rapid7Repo.findById(rapid7Id).orElse(null);
        if (rapid7 != null) {
            rapid7.setFichier(fichier);
            rapid7.setFichierOriginalName(fichierOriginalName);
            return rapid7Repo.save(rapid7);
        }
        return null;
    }
    ...
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringRapid7s() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findRapid7LicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Rapid7 rapid7 = licence.getRapid7();
                if (rapid7 != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = rapid7.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getRapid7(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Rapid7 rapid7, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = rapid7.getMailAdmin();
        List<String> ccMails = rapid7.getCcMail();

        String sujet = " Licences Rapid7 expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(rapid7.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences Rapid7 expirant " + delai);
        } catch (Exception e) {
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Rapid7 rapid7 = licence.getRapid7();
        if (rapid7 == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Rapid7 associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence Rapid7 expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(rapid7.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(rapid7.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Rapid7 expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    rapid7.getMailAdmin(),
                    rapid7.getCcMail() != null ? rapid7.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence Rapid7 expirant " + timeframe);
        } catch (Exception e) {
