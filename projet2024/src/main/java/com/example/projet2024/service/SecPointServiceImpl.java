...
public class SecPointServiceImpl implements ISecPointService {
    ...
    @Override
    public SecPoint updateSecPointFile(Long secPointId, String fichier, String fichierOriginalName) {
        SecPoint secPoint = secPointRepo.findById(secPointId).orElse(null);
        if (secPoint != null) {
            secPoint.setFichier(fichier);
            secPoint.setFichierOriginalName(fichierOriginalName);
            return secPointRepo.save(secPoint);
        }
        return null;
    }
