...
import com.example.projet2024.repository.NetskopeRepo;
...
@Service
public class NetskopeServiceImpl implements INetskopeService {
    @Autowired
    private NetskopeRepo netskopeRepo;
    @Autowired
    ...
    @Override
    public Netskope addNetskope(Netskope netskope) {
        if (netskope.getLicences() != null) {
            for (LicenceFortinet licence : netskope.getLicences()) {
                licence.setNetskope(netskope);
            }
        }
        return netskopeRepo.save(netskope);
    }

    @Override
    public Netskope updateNetskope(Netskope netskope) {
        return netskopeRepo.save(netskope);
    }

    @Override
    public Netskope retrieveNetskope(Long netskopeId) {
        return netskopeRepo.findById(netskopeId).orElse(null);
    }

    @Override
    public List<Netskope> retrieveAllNetskopes() {
        return netskopeRepo.findAll();
    }

    @Override
    public void deleteById(Long netskopeId) {
        netskopeRepo.deleteById(netskopeId);
    }

    @Override
    public void activate(Long id) {
        Netskope netskope = netskopeRepo.findById(id).orElse(null);
        if (netskope != null) {
            netskope.setApprouve(!netskope.isApprouve());
            netskopeRepo.save(netskope);
            System.out.println("Netskope ID: " + id + " est maintenant " + netskope.isApprouve());
        }
    }

    @Override
    public Netskope updateNetskopeFile(Long netskopeId, String fichier, String fichierOriginalName) {
        Netskope netskope = netskopeRepo.findById(netskopeId).orElse(null);
        if (netskope != null) {
            netskope.setFichier(fichier);
            netskope.setFichierOriginalName(fichierOriginalName);
            return netskopeRepo.save(netskope);
        }
        return null;
    }
    ...
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringNetskopes() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findNetskopeLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            // NOUVELLE LOGIQUE DES DATES (identique à Alwarebytes)
            ...
            if (shouldSend) {
                Netskope netskope = licence.getNetskope();
                if (netskope != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = netskope.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getNetskope(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Netskope netskope, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = netskope.getMailAdmin();
        List<String> ccMails = netskope.getCcMail();

        String sujet = "Licences Netskope expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(netskope.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences Netskope expirant " + delai);
        } catch (Exception e) {
            System.err.println("Erreur envoi email Netskope: " + e.getMessage());
        }
    }
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Netskope netskope = licence.getNetskope();
        if (netskope == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Netskope associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence Netskope expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(netskope.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(netskope.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Netskope expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    netskope.getMailAdmin(),
                    netskope.getCcMail() != null ? netskope.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence Netskope expirant " + timeframe);
        } catch (Exception e) {
            throw new RuntimeException("Erreur lors de l'envoi du mail individuel Netskope", e);
        }
    }
}