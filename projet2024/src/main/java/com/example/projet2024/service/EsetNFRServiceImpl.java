...
import com.example.projet2024.repository.EsetNFRRepo;
...
@Service
public class EsetNFRServiceImpl implements IEsetNFRService{
    @Autowired
    private EsetNFRRepo esetRepo;

    @Autowired
    private MailService mailService;

    @Override
    public ESETNFR addESET(ESETNFR eset) {
        ...
        ESETNFR savedEset = esetRepo.save(eset);
        return savedEset;
    }
    @Override
    public ESETNFR updateESET(ESETNFR eset) {
        return esetRepo.save(eset);
    }

    @Override
    public ESETNFR retrieveESET(Long esetid) {
        return esetRepo.findById(esetid).orElse(null) ;
    }

    @Override
public List<ESETNFR> retrieveAllESETs() {
    ...
    public void activate(Long id) {
        ESETNFR eset = esetRepo.findById(id).orElse(null);
        if (eset != null) {
   ...
   // @Scheduled(cron = "*/10 * * * * ?") // Cette tâche sera exécutée chaque jour
    public void checkForExpiringEsets() {
        List<ESETNFR> allEsets = esetRepo.findAll();
        LocalDate currentDate = LocalDate.now();
        Map<LocalDate, List<ESETNFR>> groupedEsets = new HashMap<>();

        // Grouper les produits par date d'expiration
        for (ESETNFR eset : allEsets) {
    ...
    private void sendEmailsForGroupedEsets(Map<LocalDate, List<ESETNFR>> groupedEsets) {
        for (Map.Entry<LocalDate, List<ESETNFR>> entry : groupedEsets.entrySet()) {
            LocalDate expirationDate = entry.getKey();
            List<ESETNFR> esets = entry.getValue();
            String timeframe = "";
    ...
    private String determineTimeframe(LocalDate expirationDate, ESETNFR eset) {
        LocalDate currentDate = LocalDate.now();
        long daysBetween = ChronoUnit.DAYS.between(currentDate, expirationDate);
        long monthsBetween = ChronoUnit.MONTHS.between(currentDate, expirationDate);
        long weeksBetween = ChronoUnit.WEEKS.between(currentDate, expirationDate);
    ...
    public void sendGroupedExpirationEmails(List<ESETNFR> esets, LocalDate expirationDate, String timeframe) {
    ...
    public void sendIndividualExpirationEmails(ESETNFR eset, LocalDate expirationDate, String timeframe) {
