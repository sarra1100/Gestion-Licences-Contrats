...
import com.example.projet2024.repository.ImpervaRepo;
...
@Service
public class ImpervaServiceImpl implements IImpervaService{
    @Autowired
    private ImpervaRepo impervaRepo;
    @Autowired
    ...
    @Override
    public Imperva addImperva(Imperva imperva) {
        if (imperva.getLicences() != null) {
            for (LicenceFortinet licence : imperva.getLicences()) {
                licence.setImperva(imperva);
            }
        }
        return impervaRepo.save(imperva);
    }

    @Override
    public Imperva updateImperva(Imperva imperva) {
        return impervaRepo.save(imperva);
    }

    @Override
    public Imperva retrieveImperva(Long impervaId) {
        return impervaRepo.findById(impervaId).orElse(null);
    }

    @Override
    public List<Imperva> retrieveAllImpervas() {
        return impervaRepo.findAll();
    }

    @Override
    public void deleteById(Long impervaId) {
        impervaRepo.deleteById(impervaId);
    }

    @Override
    public void activate(Long id) {
        Imperva imperva = impervaRepo.findById(id).orElse(null);
        if (imperva != null) {
            imperva.setApprouve(!imperva.isApprouve());
            impervaRepo.save(imperva);
            System.out.println("Imperva ID: " + id + " est maintenant " + imperva.isApprouve());
        }
    }

    @Override
    public void updateImpervaFile(Long id, String fichier, String fichierOriginalName) {
        Imperva imperva = impervaRepo.findById(id).orElse(null);
        if (imperva != null) {
            imperva.setFichier(fichier);
            imperva.setFichierOriginalName(fichierOriginalName);
            impervaRepo.save(imperva);
        }
    }

    // Méthode pour vérifier la date d'expiration - LOGIQUE ALWAREBYTES APPLIQUÉE
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringImpervas() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findImpervaLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Imperva imperva = licence.getImperva();
                if (imperva != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = imperva.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getImperva(), licences, timeframeDisplay);
            }
        }
    }
    ...
    // Méthode modifiée pour accepter le paramètre delai
    private void sendGroupedExpirationEmails(Imperva imperva, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = imperva.getMailAdmin();
        List<String> ccMails = imperva.getCcMail();

        String sujet = "Licences Imperva expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(imperva.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email Imperva envoyé pour " + licencesExpirant.size() + " licences expirant " + delai);
        } catch (Exception e) {
            System.err.println("Erreur envoi email Imperva: " + e.getMessage());
        }
    }
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Imperva imperva = licence.getImperva();
        if (imperva == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Imperva associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(imperva.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(imperva.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Imperva expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    imperva.getMailAdmin(),
                    imperva.getCcMail() != null ? imperva.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel Imperva envoyé pour la licence expirant " + timeframe);
        } catch (Exception e) {
            throw new RuntimeException("Erreur lors de l'envoi du mail individuel Imperva", e);
        }
    }
}