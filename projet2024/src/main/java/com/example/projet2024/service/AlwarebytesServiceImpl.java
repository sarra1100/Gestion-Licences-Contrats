...
import com.example.projet2024.repository.AlwarebytesRepo;
...
@Service
public class AlwarebytesServiceImpl implements IAlwarebytesService {
    @Autowired
    private AlwarebytesRepo alwarebytesRepo;
    @Autowired
    ...
    @Override
    public Alwarebytes addAlwarebytes(Alwarebytes alwarebytes) {
        if (alwarebytes.getLicences() != null) {
            for (LicenceFortinet licence : alwarebytes.getLicences()) {
                licence.setAlwarebytes(alwarebytes);
            }
        }
        return alwarebytesRepo.save(alwarebytes);
    }

        @Override
        public Alwarebytes updateAlwarebytesFile(Long id, String fichier, String fichierOriginalName) {
            Alwarebytes alwarebytes = alwarebytesRepo.findById(id).orElse(null);
            if (alwarebytes != null) {
                alwarebytes.setFichier(fichier);
                alwarebytes.setFichierOriginalName(fichierOriginalName);
                return alwarebytesRepo.save(alwarebytes);
            }
            return null;
        }
    ...
    @Override
    public Alwarebytes updateAlwarebytes(Alwarebytes alwarebytes) {
        return alwarebytesRepo.save(alwarebytes);
    }

    @Override
    public Alwarebytes retrieveAlwarebytes(Long alwarebytesId) {
        return alwarebytesRepo.findById(alwarebytesId).orElse(null);
    }

    @Override
    public List<Alwarebytes> retrieveAllAlwarebytess() {
        return alwarebytesRepo.findAll();
    }

    @Override
    public void deleteById(Long alwarebytesId) {
        alwarebytesRepo.deleteById(alwarebytesId);
    }
    ...
    public void activate(Long id) {
        Alwarebytes alwarebytes = alwarebytesRepo.findById(id).orElse(null);
        if (alwarebytes != null) {
            alwarebytes.setApprouve(!alwarebytes.isApprouve());
            alwarebytesRepo.save(alwarebytes);
            System.out.println("Alwarebytes ID: " + id + " est maintenant " + alwarebytes.isApprouve());
        }
    }

    // Méthode pour vérifier la date d'expiration
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringAlwarebytess() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findAlwarebytesLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Alwarebytes alwarebytes = licence.getAlwarebytes();
                if (alwarebytes != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = alwarebytes.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getAlwarebytes(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Alwarebytes alwarebytes, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = alwarebytes.getMailAdmin();
        List<String> ccMails = alwarebytes.getCcMail();

        String sujet = " Licences Alwarebytes expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(alwarebytes.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences expirant " + delai);
        } catch (Exception e) {
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Alwarebytes alwarebytes = licence.getAlwarebytes();
        if (alwarebytes == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Alwarebytes associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence Alwarebytes expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(alwarebytes.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(alwarebytes.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Alwarebytes expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    alwarebytes.getMailAdmin(),
                    alwarebytes.getCcMail() != null ? alwarebytes.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence expirant " + timeframe);
        } catch (Exception e) {
