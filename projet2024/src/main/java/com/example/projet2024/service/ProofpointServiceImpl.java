...
import com.example.projet2024.repository.ProofpointRepo;
...
@Service
public class ProofpointServiceImpl implements IProofpointService {

    @Autowired
    private ProofpointRepo proofpointRepo;
    @Autowired
    ...
    @Override
    public Proofpoint addProofpoint(Proofpoint proofpoint) {
        if (proofpoint.getLicences() != null) {
            for (LicenceFortinet licence : proofpoint.getLicences()) {
                licence.setProofpoint(proofpoint);
            }
        }
        return proofpointRepo.save(proofpoint);
    }

    @Override
    public Proofpoint updateProofpoint(Proofpoint proofpoint) {
        return proofpointRepo.save(proofpoint);
    }

    @Override
    public Proofpoint retrieveProofpoint(Long proofpointId) {
        return proofpointRepo.findById(proofpointId).orElse(null);
    }

    @Override
    public List<Proofpoint> retrieveAllProofpoints() {
        return proofpointRepo.findAll();
    }

    @Override
    public void deleteById(Long proofpointId) {
        proofpointRepo.deleteById(proofpointId);
    }

    @Override
    public void activate(Long id) {
        Proofpoint proofpoint = proofpointRepo.findById(id).orElse(null);
        if (proofpoint != null) {
            proofpoint.setApprouve(!proofpoint.isApprouve());
            proofpointRepo.save(proofpoint);
            System.out.println("Proofpoint ID: " + id + " est maintenant " + proofpoint.isApprouve());
        }
    }

    @Override
    public Proofpoint updateProofpointFile(Long proofpointId, String fichier, String fichierOriginalName) {
        Proofpoint proofpoint = proofpointRepo.findById(proofpointId).orElse(null);
        if (proofpoint != null) {
            proofpoint.setFichier(fichier);
            proofpoint.setFichierOriginalName(fichierOriginalName);
            return proofpointRepo.save(proofpoint);
        }
        return null;
    }
    ...
    @Scheduled(cron = "*/10 * * * * ?")
    public void checkForExpiringProofpoints() {
        LocalDate today = LocalDate.now();
        LocalDate maxDate = today.plusMonths(6);

        List<LicenceFortinet> expiringLicences = licenceFortinetRepo.findProofpointLicencesExpiringBetween(today, maxDate);
        ...
        for (LicenceFortinet licence : expiringLicences) {
            ...
            if (shouldSend) {
                Proofpoint proofpoint = licence.getProofpoint();
                if (proofpoint != null) {
                    String timeframe = determineTimeframe(daysUntilExpiration);
                    String groupKey = proofpoint.getClient() + "_" + timeframe;
        ...
        for (Map.Entry<String, List<LicenceFortinet>> entry : groupedLicencesByClientAndTimeframe.entrySet()) {
            ...
            if (!licences.isEmpty()) {
                ...
                sendGroupedExpirationEmails(licences.get(0).getProofpoint(), licences, timeframeDisplay);
            }
        }
    }
    ...
    private void sendGroupedExpirationEmails(Proofpoint proofpoint, List<LicenceFortinet> licencesExpirant, String delai) {
        String destinataire = proofpoint.getMailAdmin();
        List<String> ccMails = proofpoint.getCcMail();

        String sujet = "Licences Proofpoint expirant " + delai;
        ...
        for (LicenceFortinet licence : licencesExpirant) {
            contenu.append("<tr>")
                    .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(proofpoint.getClient()).append("</td>")
        ...
        try {
            ...
            System.out.println("Email envoyé pour " + licencesExpirant.size() + " licences Proofpoint expirant " + delai);
        } catch (Exception e) {
            System.err.println("Erreur envoi email Proofpoint: " + e.getMessage());
        }
    }
    ...
    public void sendIndividualExpirationEmails(LicenceFortinet licence) {
        Proofpoint proofpoint = licence.getProofpoint();
        if (proofpoint == null) {
            throw new IllegalArgumentException("La licence ne contient pas d'objet Proofpoint associé");
        }
        ...
        emailBody.append("<p>Bonjour,</p>")
                .append("<p>Vous trouverez ci-dessous les détails de la licence Proofpoint expirant ")
                .append(timeframe)
                ...
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(proofpoint.getNomDuContact()).append("</td>")
                .append("<td style='border: 1px solid #ff9933; padding: 8px;'>").append(proofpoint.getAdresseEmailContact()).append("</td>")
                .append("</tr>")
                .append("</table>");

        String subject = "Alerte: Licence Proofpoint expirant " + timeframe;

        try {
            emailService.sendEsetNotification(
                    proofpoint.getMailAdmin(),
                    proofpoint.getCcMail() != null ? proofpoint.getCcMail() : List.of(),
                    subject,
                    emailBody.toString()
            );
            System.out.println("Email individuel envoyé pour la licence Proofpoint expirant " + timeframe);
        } catch (Exception e) {
            throw new RuntimeException("Erreur lors de l'envoi du mail individuel Proofpoint", e);
        }
    }
}